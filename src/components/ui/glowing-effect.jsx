...
const GlowingEffect = memo(({
  ...
  const handleMove = useCallback((e) => {
    ...
    animationFrameRef.current = requestAnimationFrame(() => {
      ...
      if (distance < inactiveR) { element.style.setProperty('--active', '0'); return; }
  ...
  return (
    <div
      ...
      style={{
        ...
        '--gradient': 'radial-gradient(circle, #2563EB 10%, #2563EB00 20%), radial-gradient(circle at 40% 40%, #F59E0B 10%, #F59E0B00 20%), radial-gradient(circle at 60% 60%, #60A5FA 10%, #60A5FA00 20%), repeating-conic-gradient(from 236.84deg at 50% 50%, #2563EB 0%, #F59E0B 5%, #60A5FA 10%, #2563EB 20%)',
      }}
      className={cn(
        'pointer-events-none absolute inset-0 rounded-[inherit] opacity-100 transition-opacity',
        glow && 'opacity-100', blur > 0 && 'blur-[var(--blur)]', className
      )}
    >
      <div
        className={cn(
